 ...
 *
 *  Original code from takissd
 *    https://github.com/takissd/MySmartThings/tree/master/devicetypes/takis/smartweather-station-tile-2-0.src
 *  
 * 
 *  VERSION HISTORY
 *
 *   v2.1 (30-Oct-2016): change device name so it can be referred to by 'device.smartWeatherStationTile2'
 *
*/
metadata {
	definition (name: "VIRTUAL Smart Weather Station Tile 2", namespace: "astrowings", author: "Phil Maynard") {
...
def poll() {
	...
	if (obs) {
        log.debug "obs --> ${obs}"
        def now = new Date().format('HH:mm:ss dd/MM/yyyy',location.timeZone)
